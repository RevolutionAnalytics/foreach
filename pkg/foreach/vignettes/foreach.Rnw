...
Beginning with \texttt{foreach} 1.5.2, \texttt{foreach} can use the
\texttt{future} package, if available, to automatically detect needed 
packages. Use \texttt{options(foreachGlobals="future")} or
\texttt{options(foreachGlobals="foreach+future")} to activate this option.
(The \texttt{future} package also helps to find symbols that may have been missed
by \texttt{foreach}'s internal algorithm.)

If you are using Microsoft R Open, the future package is used automatically, if 
it is installed and can be loaded. 
You can suppress this by setting \texttt{options(foreachGlobals="foreach")}.

To unset the \texttt{foreachGlobals} option, use \texttt{options(foreachGlobals=NULL)}.
