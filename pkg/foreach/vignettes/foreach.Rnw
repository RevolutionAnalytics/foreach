...
Beginning with \texttt{foreach} 0.5.0, \texttt{foreach} will use the
\texttt{future} package, if available, to automatically detect needed 
packages. (This also requires appropriately updated \texttt{foreach} backends.)
You can retain the old behavior by setting \texttt{options(foreachGlobals="foreach")}.
